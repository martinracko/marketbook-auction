...
class Crawler(QWebView):
    ...
    def _loadFinished(self):
        ...
        if soup.title != None:
            ...
            try:
                ...
                elif '/models.aspx' in url:
                    self.parseModelList(soup)
                elif '/list.aspx' in url:
                    self.parseList(soup)
                elif '/detail.aspx' in url or 'Details.aspx' in url:
    ...
    def loadNextPage(self):
        ...
        if self.nextPage != None:
            ...
            if len(self.categories) > 0:
                self.nextPage = self.categories.pop(0)
            elif len(self.modelList) > 0:
    ...
    def parseCategories(self, soup):
        ...
        links = soup.find_all("a", {"class": "headinglink"})
        cnt = len(links)
        for i in range(0, cnt):
            url = str(self.baseUrl + links[i]['href']).replace('etid=1', 'etid=5')
            if 'manufacturers.aspx' in url:
                self.categories.append(url)
        if cnt == 0:
            self.terminate('No categories to parse in category list')
        ...
        if mans != None:
            ...
            for i in range(0, cnt):
                ...
                if listingsCnt < 10000:
                    self.sitemap.append(self.baseUrl + href.replace('drilldown/models.aspx', 'list/list.aspx'))
                else:
    ...
    def parseList(self, soup):
        ...
        duplicatesCnt = 0
        for i in range(0, cnt):
            url = self.baseUrl + links[i]['href'] if self.baseUrl not in links[i]['href'] else links[i]['href']
    ...
    def log(self, message):
        if self.cfg.getboolean('log', 'log'):
            doc = {'date': datetime.datetime.utcnow(),
                   ...
                   'message': message}
            self.db['log.marketbook-auction'].insert(doc)
...
if __name__ == '__main__':
    cfg = CrawlerConfig()
    cfg.read('marketbook-auction.ini')
